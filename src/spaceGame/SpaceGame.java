...
public class SpaceGame implements ActionListener {
	...
	public ArrayList<Text> textCache = new ArrayList<Text>();
	...
	//Timers
	public Timer changeText = new Timer(1500,this);
	...
	public Timer bossAttack = new Timer(6000,this);
	public Timer easyTimer = new Timer(10000, this); //for rounds lasting too long maybe
	public Timer printLetter = new Timer(50,this);
	public Timer BossCD = new Timer(2000,this);
	
		
	//images
	public BufferedImage howToPlayMenu, questionButton,title, playButton, quitButton, image, quitButton2, image2,playButton2, tutorialButton,wave1Button,wave2Button,wave3Button,wave4Button,bossButton, selectorImage;
	...
	*/
	void setup_game() {
		...
		prints.add("to the");
		prints.add("TUTORIAL");
		prints.add("try pressing wasd or the arrow keys to move");
		prints.add("Nice!");
		prints.add("now try pressing space bar to boost");
		prints.add("GOOD JOB!!!!");
		prints.add("AN ENEMY HAS APPEARED");
		prints.add("SHOOTING HAS BEGAN");
	}
	...
	*/
	...
	void func_winner() {
		...
		winner.drawString(String.format("KILLS: %d", kills), WINB/6, WINH/2+75);
		winner.sleep(3000);
		kills = 0;
		winner.dispose();
		new SpaceGame();
	...
	*/
	void func_startGame() {
			...
			easyTimer.start();
			printLetter.start();
			if (level != 5) enemySpawn.start();
			if (level == 0) changeText.start();
	}
	...
	*/
	...
	void updTUTORIAL() {
		...
		if (loop == 3) {
			changeText.restart(); changeText.stop();
		...
		if (loop == 7) {
			enemyCache.add(new Liner(300,300));
			changeText.restart(); changeText.stop();
			loop+=1;
		}
		if (loop == 8) {
	...
	*/
	void drawGraphics() {
		synchronized(game) {
			...
			//draws text for tutorial level
			if (level == 0) {
				game.setColor(Color.WHITE);
				for (Text t: textCache) {
					game.drawString(t.text, t.x,t.y);
				}
			}
			
			//draws powerup if there is more than 0
			game.setColor(Color.WHITE);
			if (powerCache.size() > 0) {
				for (Powerups p: powerCache) {
					game.fillRect(p.x,p.y,p.diameter,p.diameter);
				}
			}
			...
			if (level == 5) {
				game.setColor(Color.WHITE);
				game.fillRect(Boss.x, Boss.y, Boss.size ,Boss.size);
			}
